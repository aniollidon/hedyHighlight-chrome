...
export function main() {
    ...
    if (event.data.type === "HEDY_CODE_UPDATE") {
      ...
      if (event.data.level && event.data.level !== level) {
        ...
        try {
          ...
          console.error(
            "Hedy Error Highlighter: Error re-initializing CheckHedy with new level:",
            e
          );
        }
      }
  ...
  const analyzeCode = (text) => {
    ...
    try {
      ...
      console.error(
        "Hedy Error Highlighter: Error re-initializing CheckHedy:",
        e
      );
      return;
    }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      try {
        ...
        if (errors && errors.length > 0) {
          console.log(
            `Hedy Error Highlighter: Found ${errors.length} errors on line ${i}:`,
            errors
          );
        ...
        console.error(
          `Hedy Error Highlighter: Error analyzing line ${i + 1}:`,
          e
        );
      }
    }
    ...
    window.postMessage(
      { type: "HEDY_HIGHLIGHT_ERRORS", errors: allErrors },
      "*"
    );
  };
}
