function detectPrintUsages(tokens) {
  ...
  while (i < tokens.length) {
    ...
    if (
      i + 1 < tokens.length &&
      (tokens[i].command === "print" || tokens[i].command === "echo")
    ) {
...
function detectDeclarations(tokens) {
  ...
  while (i < tokens.length) {
    ...
    if (
      i + 1 < tokens.length &&
      (tokens[i + 1].command === "variable_define_is" ||
        tokens[i + 1].command === "variable_define_equal")
    ) {
...
function detectLanguageFunctions(
  tokens,
  hasAtRandom = false,
  hasRange = false
) {
...
function detectFuctionUsages(tokens) {
  ...
  while (i < tokens.length) {
    ...
    if (tokens[i].command === "call") {
      ...
      const tag =
        phrase[1] && phrase[1].tag.startsWith("entity_function_return")
...
function detectConditions(tokens) {
  ...
  while (i < tokens.length) {
    if (
      ...
      if (
        i + 2 < tokens.length &&
        (!tokens[i + 2].command || tokens[i + 2].command === "pressed")
      ) {
        phrase.push(tokens[i + 2]);
        count++;
      }
