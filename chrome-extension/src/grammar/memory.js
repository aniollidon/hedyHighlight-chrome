...
class Memory {
  ...
  newSintagma(words, identation, lineNumber) {
    ...
    if (words[0].command === "if" || words[0].command === "elif")
      sintagmaTag = "condition";
  ...
  getDefinedIdentation(ifnotdef = 4) {
    return this._definedScopeIdentation !== -1
      ? this._definedScopeIdentation
      : ifnotdef;
  }
  ...
  comprovaScope(identation) {
    ...
    if (identation === identPast) {
      ...
      if (pastIdentable)
        if (
          this._definedScopeIdentation != -1 &&
          identPast + this._definedScopeIdentation !== identation
        )
          return "large";
        else return true;
      return "not_expected";
