  ...
  _searchMorphosyntacticErrors(sintagma) {
    ...
    for (let k = 0; k < sintagma.size(); k++) {
      ...
      if (word.command) {
        ...
        if (
          ...
          if (commandDef.usesParameters) {
            if (
              k + 1 < sintagma.size() &&
              sintagma.get(k + 1).entity &&
              sintagma.get(k + 1).entity.params
            ) {
          ...
          } else if (Array.isArray(commandDef.argumentsAfter))
          ...
          for (let j = endArgsMax + 1; j < sintagma.size(); j++) {
            if (
              commandDef.closedBy &&
              j == sintagma.size() - 1 &&
              commandDef.closedBy === sintagma.get(j).command
            ) {
              sintagma.markUsed(j);
              continue;
            } else if (
          ...
          if (
            commandDef.closedBy &&
            commandDef.closedBy !== sintagma.last().command
          ) {
            errorsFound.push(
              new HHErrorVal(
                word.text,
        ...
        for (
          let j = startArgsCommand;
          j < endArgsCommand + 1 && j < sintagma.size();
          j++
        ) {
          sintagma.markUsed(j);
        }
        ...
        if (commandDef.arguments) {
          for (let sx = 0; sx < commandDef.arguments.length; sx++) {
            ...
            if (rule.closedBy) {
              if (sintagma.last().tag === rule.closedBy)
              ...
              for (
                ...
                if (
                  rule.positionInSintagma !== undefined &&
                  rule.positionInSintagma !== k
                )
                  continue;
                if (
                  rule.position !== undefined &&
                  rule.position !== j - startArgsCommand
                )
                  continue;
      ...
      for (let i = 0; i < generalSyntax.length; i++) {
        ...
        errorsFound.push(
          new HHErrorType(word.text, rule.codeerror, start, end, word.tag)
        );
      }
    }
    ...
    for (let k = 0; k < sintagma.size(); k++) {
      ...
      for (let j = 0; j < specificErrors.length; j++) {
        ...
        if (error.beforeAndAfter && error.beforeAndAfter === "same") {
          ...
          if (k === 0 || k + 1 >= sintagma.size()) continue;
          if (compareTypes(sintagma.get(k - 1).tag, sintagma.get(k + 1).tag))
            continue;
        } else if (
          error.beforeAndAfter &&
          ...
          if (k === 0 || k + 1 >= sintagma.size()) continue;
          if (
            sintagma.get(k - 1).constant === undefined &&
            sintagma.get(k + 1).constant === undefined
          )
            continue;
          if (sintagma.get(k - 1).constant !== sintagma.get(k + 1).constant)
            continue;
        }
        ...
        if (error.highlight === "before_word") {
          ...
          for (let i = 0; i + 1 < sintagma.size(); i++) {
            if (sintagma.get(i).tag.startsWith("command_variable_define"))
